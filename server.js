...
app.post("/webhook/stripe", bodyParser.raw({ type: "application/json" }), async (req, res) => {
  ...
  switch (event.type) {
    ...
    case "customer.subscription.updated":
      await updateUserByCustomer(data.customer, data.status); break;
    case "customer.subscription.deleted":
      await updateUserByCustomer(data.customer, "canceled"); break;
    case "invoice.payment_failed":
      await updateUserByCustomer(data.customer, "past_due"); break;
  }
...
app.post("/api/activate-master-key", requireAuth, attachUser, async (req, res) => {
  ...
  if (!key) return res.status(404).json({ error: "Invalid master key" });
  if (key.usedBy.includes(user.userId)) return res.status(400).json({ error: "Key already used by this account" });
  if (key.usedBy.length >= key.maxUses) return res.status(400).json({ error: "Key usage limit reached" });
...
app.post("/api/inspections/create", requireAuth, attachUser, enforceSubscription, async (req, res) => {
  const user = req.user;
  const { hiveId, data } = req.body;
  if (!hiveId || !data) return res.status(400).json({ error: "Missing hiveId or data" });
  res.json({ ok: true, message: "Inspection saved", userId: user.userId, hiveId, data });
});

app.post("/api/inspections/export", requireAuth, attachUser, enforceSubscription, async (req, res) => {
  const user = req.user;
  res.json({ ok: true, message: "Export ready", userId: user.userId });
});

app.get("/api/stripe/create-checkout", requireAuth, attachUser, async (req, res) => {
  const user = req.user;
  const { plan } = req.query;
  const prices = {
    hobbyist_monthly: process.env.STRIPE_HOBBYIST_MONTHLY,
    hobbyist_annual: process.env.STRIPE_HOBBYIST_ANNUAL,
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: `${process.env.FRONTEND_URL || "https://beebuddi.co.uk"}?payment=success`,
    cancel_url: `${process.env.FRONTEND_URL || "https://beebuddi.co.uk"}?payment=cancelled`,
